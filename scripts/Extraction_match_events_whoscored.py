...
def get_match_events(driver):
    ...
    # Extrai mandante e visitante com base nos <a class="team-link">
    team_links = soup.find_all("a", class_="team-link")
    home_team = team_links[0].text.strip()
    away_team = team_links[1].text.strip()
    
    # Resultado padronizado (formato A-A)
    score = soup.find("td", class_="result").text.strip().replace(" ", "").replace(":", "-")

    # Data
    match_date = soup.find("dt", text="Date:").find_next_sibling("dd").text.strip()

    # IdJogo: TimeMandante x TimeVisitante
    id_jogo = f"{home_team} x {away_team}"
